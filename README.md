...
GitHub Actions (twice daily: midnight + noon Central)
  ...
  -> Generates docs/picks.xml (WYXR Picks RSS feed, next 180 days)
...
Vercel (Frontend)
  ...
  -> Serves docs/picks.xml (WYXR Picks RSS feed, linked from the public footer)
...
Two RSS 2.0 feeds are published. Both are **public and unauthenticated**, and both are regenerated by every build (twice daily).

| Feed | Contents | Window | Linked from |
|---|---|---|---|
| [`/feed.xml`](https://concert-calendar.wyxr.org/feed.xml) | every event, plus sponsor callouts | 60 days | Admin → Tools → Feeds, and `<link rel="alternate">` autodiscovery |
| [`/picks.xml`](https://concert-calendar.wyxr.org/picks.xml) | **WYXR Picks only** | 180 days | the public footer |

`/feed.xml` is the feed the WYXR app and general feed readers consume. Each item carries artist, venue, date, time, price, genre, and the WYXR Presents/Pick badges.

`/picks.xml` is the same item format filtered to events whose **WYXR Pick** flag (`is_featured`) is set — a curated recommendation list, so it carries **no sponsor items**. The wider 180-day window is deliberate: picks are sparse, and a subscriber wants the whole slate rather than a 60-day slice. An event that is *also* WYXR Presents appears here; WYXR Presents alone does not, since the two flags are independent.
...
│   ├── picks.xml              # WYXR Picks RSS feed (auto-generated, 180-day window)
...
```bash
./test_before_push.sh     # expect 18/18
```
...
| `test_admin_hover_browser.py` | the admin list's row-hover preview card — content, placement, escaping |
...
rather than the "all checks passed" line — skipping all three reports 15/15 instead of 18/18
